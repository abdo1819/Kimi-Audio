#!/usr/bin/env python
"""
UPDATED *again* – robust length fetching **plus RESUME‑SUPPORT & RUNTIME‑ROBUSTNESS**
----------------------------------------------------------------
Adds automatic W&B‑run resumption **and** makes the main loop resilient to rare
`torch.cat(): expected a non‑empty list of Tensors` crashes coming from the
Whisper tokenizer when the audio contains no speech (or is otherwise
malformed).

Key additions compared to the previous version
==============================================
1. **Resume logic** (unchanged from previous patch): `.wandb_run_id` file,
   `resume="allow"`, helper resume command.
2. **Safe transcription**:
   * Before calling the model we verify the audio *duration* is at least
     0.2 s (≃ 3 200 samples at 16 kHz).  Shorter clips are skipped and logged.
   * `model.generate` is wrapped in a `try/except RuntimeError` block.  When
     we hit the known Whisper bug we log the failure, count the reference as
     an **insertion‑only** hypothesis (empty string) so WER keeps the correct
     denominator, and keep going instead of aborting the whole script.
   * A `failed` counter and optional `fail_table` artifact are logged to W&B
     so you can inspect which files caused issues.
3. **Graceful finish** still removes the resume‑file; crashes/interruption do
   not.

You can now launch, Ctrl‑C, resume, or hit Whisper edge‑cases without losing
progress.
"""

from __future__ import annotations

import os, re, csv, argparse, tempfile, difflib, sys, shlex, math
...
# -----------------------------------------------------------------------------
results_p, results_r, mistakes, failures = [], [], [], []
...
try:
    for idx, ex in enumerate(tqdm(ds, total=pbar_total, desc="Transcribing")):
        # Skip too-short audio (often produces empty feature list)
        n_samples = len(ex["audio"]["array"])
        if n_samples < 0.2 * ex["audio"]["sampling_rate"]:  # <200 ms
            failures.append({"index": idx, "reason": "too_short", "ref_text": ex.get("text","")})
            results_p.append("")
            results_r.append(_clean(ex["text"]))
            continue
        ...
        try:
            try:
                ...
                if "expected a non-empty list" in str(e):
                    failures.append({"index": idx, "reason": "whisper_empty", "error": str(e)})
                    out = ""  # Treat as empty hypothesis so insertion counts
                else:
                    raise
        finally:
            os.remove(path)
...
if mistakes:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        csv.DictWriter(f, fieldnames=mistakes[
